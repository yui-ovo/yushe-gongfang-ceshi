...
for (const wording of [
  ...
  '所以旧 G 和旧的同名空变量项会继续保留',
  '当前找到 ${preview.oldGetOccurrences} 处 G、${preview.oldEmptySetOccurrences} 处同名空变量项（只有变量名，无内容）',
...
console.log('test.20 回归通过：变量弹窗使用“同名空变量项（只有变量名，无内容）”，改名说明已改为大白话。');
