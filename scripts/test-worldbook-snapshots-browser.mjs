...
try {
  for (const width of [360, 390, 1280]) {
    ...
    await page.click('[data-wbs="back-sources"]');
    assert.equal(await page.getByText('已有快照的分组',{exact:true}).count(),0,'No saved snapshot group before first capture');
    assert.equal(await page.locator('[data-wbs="new"]').count(),0,'Picker back restores snapshot landing page, not group detail');
    await page.click('[data-wbs="sources"]');
    ...
    await page.click('[data-wbs="back-sources"]');
    await page.getByText('已有快照的分组',{exact:true}).waitFor();
    assert.equal(await page.locator('[data-wbs="new"]').count(),0,'Picker back with saved groups stays on landing page');
