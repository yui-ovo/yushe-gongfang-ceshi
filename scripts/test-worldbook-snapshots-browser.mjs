...
try {
  for (const width of [360, 390, 1280]) {
    ...
    await page.locator('[data-wbs="choose"]').first().click();
    await page.click('[data-wbs="new"]');
    ...
    await page.locator('[data-filter]').fill('剧情推进');
    assert.equal(await page.locator('[data-draft-book][open]').count(),2,'Search opens matches in both books');
    await page.locator('[data-filter]').fill('');
    assert.equal(await page.locator('[data-draft-book="剧情补充"]').getAttribute('open'),null,'Clearing search restores collapse');
    await page.locator('[data-draft-book="剧情补充"] summary').click();
    ...
    await page.waitForFunction(()=>document.querySelector('[data-message]').hidden,{},{timeout:5000});
    ...
    await page.click('[data-hub-tab="global"]');
    await page.click('[data-wbs="snapshots"]');
    await page.getByText('已有快照的分组',{exact:true}).waitFor();
